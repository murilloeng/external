...
subroutine chebyshev_set ( n, x, w )
...
!
  ...
  if ( n == 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'CHEBYSHEV_SET - Fatal error!'
...
!
  ...
  w(1:n) = r8_pi / real ( n, kind = rk )

  do i = 1, n
    x(i) = cos ( r8_pi * real ( 2 * n + 1 - 2 * i, kind = rk ) &
                       / real ( 2 * n, kind = rk ) )
  end do
...
!
  if ( mod ( expon, 2 ) == 0 ) then
    ...
    exact = r8_pi * real ( top, kind = rk ) / real ( bot, kind = rk )
...
!
  ...
  do i = 1, n
    angle = r8_pi * real ( n + 1 - i, kind = rk ) / real ( n + 1, kind = rk )
    w(i) = r8_pi / real ( n + 1, kind = rk ) * ( sin ( angle ) )**2
    x(i) = cos ( angle )
  end do
...
!
  ...
  do i = 1, n
    ...
    angle = real ( n - i, kind = rk ) * r8_pi &
          / real ( n - 1, kind = rk )
    x(i) = cos ( angle )

  end do

  w(1)     = r8_pi / real ( 2 * ( n - 1 ), kind = rk )
  w(2:n-1) = r8_pi / real (       n - 1,   kind = rk )
  w(n)     = r8_pi / real ( 2 * ( n - 1 ), kind = rk )

  return
end
subroutine chebyshev3_integral ( expon, exact )
...
!
  ...
  if ( 500.0D+00 * temp < abs ( ( gamma ( temp2 ) )**2 - pi ) ) then
    write ( *, '(a)' ) ' '
...
!
  ...
  do i = 1, n
    theta(i) = real ( n - i, kind = rk ) * r8_pi &
             / real ( n - 1, kind = rk )
  end do
...
!
  ...
  do i = 1, n
    theta(i) = real ( 2 * ( n + 1 - i ) - 1, kind = rk ) * r8_pi &
             / real ( 2 * n,     kind = rk )
  end do
  ...
  do i = 1, n
    ...
    do j = 1, ( n / 2 )
      w(i) = w(i) - 2.0D+00 &
        * cos ( 2.0D+00 * real ( j, kind = rk ) * theta(i) ) &
        / real ( 4 * j * j - 1, kind = rk )
    end do
  end do

  w(1:n) = 2.0D+00 * w(1:n) / real ( n, kind = rk )
...
!
  ...
  do i = 1, n
    theta(i) = real ( n + 1 - i, kind = rk ) * r8_pi &
             / real ( n + 1, kind = rk )
  end do
  ...
  do i = 1, n
    ...
    if ( 2 < n ) then
      p = 2.0D+00 * real ( ( ( n + 1 ) / 2 ), kind = rk ) - 1.0D+00
      w(i) = w(i) - cos ( ( p + 1.0D+00 ) * theta(i) ) / p
    end if

  end do

  w(1:n) = 2.0D+00 * w(1:n) / real ( n + 1, kind = rk )
...
!    Input, integer EXPON, the exponent.
!
!    Input, real ( kind = rk ) ALPHA, the exponent of (1-X*X) in the weight.
!    -1.0 < ALPHA.
!
...
subroutine gegenbauer_ek_compute ( n, alpha, a, b, x, w )
...
!    -1.0 < ALPHA.
!
!    Input, real ( kind = * ) A, B, the left and right endpoints 
!    of the interval.
!
!    Output, real ( kind = rk ) X(N), the abscissas.
!
...
!  Parameters:
!
!    Output, real ( kind = rk ) P2, the value of J(N)(X).
!
!    Output, real ( kind = rk ) DP2, the value of J'(N)(X).
!
!    Output, real ( kind = rk ) P1, the value of J(N-1)(X).
!
...
!    -1.0 < ALPHA.
!
!    Input, real ( kind = rk ) C(N), the recursion coefficients.
!
  implicit none
...
!    Input, integer N, the order of the polynomial.
!
!    Input, real ( kind = rk ) ALPHA, the exponent of (1-X^2) in the weight.
!    -1.0 < ALPHA.
!
!    Output, real ( kind = rk ) DP2, the value of J'(N)(X).
!
...
!    Input, integer N, the order.
!
!    Input, real ( kind = rk ) ALPHA, the parameter.
!    -1.0 < ALPHA.
!
!    Output, real ( kind = rk ) X(N), the abscissas.
!
...
!
  ...
  real ( kind = rk ) alpha
  real ( kind = rk ) alpha_laguerre
  real ( kind = rk ) arg
  integer n_laguerre
  real ( kind = rk ) w(n)
  real ( kind = rk ), allocatable, dimension ( : ) :: w_laguerre
  real ( kind = rk ) x(n)
  real ( kind = rk ), allocatable, dimension ( : ) :: x_laguerre
!
...
!    0 <= EXPON.
!
!    Input, real ( kind = rk ) ALPHA, the exponent of |X| in the
...
!    0 <= EXPON.
!
!    Input, real ( kind = rk ) ALPHA, the exponent of X in the weight function.
!
...
!
  ...
  arg = alpha + real ( expon + 1, kind = rk )
...
!
  ...
  cc = gamma ( alpha + 1.0D+00 ) * product ( c(2:n) )
...
!
    if ( i == 1 ) then
      ...
      xval = ( 1.0D+00 + alpha ) * ( 3.0D+00 + 0.92 * alpha ) / &
        ( 1.0D+00 + 2.4D+00 * real ( n, kind = rk ) + 1.8D+00 * alpha )

    else if ( i == 2 ) then

      xval = xval + ( 15.0D+00 + 6.25D+00 * alpha ) / &
        ( 1.0D+00 + 0.9D+00 * alpha + 2.5D+00 * real ( n, kind = rk ) )
...
!  Parameters:
!
!    Output, real ( kind = rk ) P2, the value of L(N)(X).
!
!    Output, real ( kind = rk ) DP2, the value of L'(N)(X).
!
!    Output, real ( kind = rk ) P1, the value of L(N-1)(X).
!
!    Input, real ( kind = rk ) X, the point at which polynomials are evaluated.
!
!    Input, integer N, the order of the polynomial.
...
!    integrand.
!
!    Input, real ( kind = rk ) B(N), C(N), the recursion coefficients.
!
  implicit none
...
subroutine hermite_probabilist_set ( n, x, w )
...
!
  ...
  if ( n == 1 ) then
    ...
    x(2) = +1.00000000000000000000000000000D+00

    w(1) = 0.50000000000000000000000000000D+00
    w(2) = 0.50000000000000000000000000000D+00

  else if ( n == 3 ) then

    x(1) = -1.73205080756887729352744634151D+00
    x(2) =  0.0D+00
    x(3) = +1.73205080756887729352744634151D+00

    w(1) = 0.166666666666666666666666666667D+00
    w(2) = 0.666666666666666666666666666667D+00
    w(3) = 0.166666666666666666666666666667D+00

  else if ( n == 4 ) then

    x(1) = -2.33441421833897723931751226721D+00
    x(2) = -0.741963784302725857648513596726D+00
    x(3) = +0.741963784302725857648513596726D+00
    x(4) = +2.33441421833897723931751226721D+00

    w(1) = 0.0458758547680684918168929937745D+00
    w(2) = 0.454124145231931508183107006225D+00
    w(3) = 0.454124145231931508183107006225D+00
    w(4) = 0.0458758547680684918168929937745D+00

  else if ( n == 5 ) then

    x(1) = -2.85697001387280565416230426401D+00
    x(2) = -1.35562617997426586583052129087D+00
    x(3) =  0.0D+00
    x(4) = +1.35562617997426586583052129087D+00
    x(5) = +2.85697001387280565416230426401D+00

    w(1) = 0.0112574113277206889333702151856D+00
    w(2) = 0.222075922005612644399963118148D+00
    w(3) = 0.533333333333333333333333333333D+00
    w(4) = 0.222075922005612644399963118148D+00
    w(5) = 0.0112574113277206889333702151856D+00

  else if ( n == 6 ) then

    x(1) = -3.32425743355211895236183546247D+00
    x(2) = -1.88917587775371067550566789858D+00
    x(3) = -0.616706590192594152193686099399D+00
    x(4) = +0.616706590192594152193686099399D+00
    x(5) = +1.88917587775371067550566789858D+00
    x(6) = +3.32425743355211895236183546247D+00

    w(1) = 0.00255578440205624643060629074383D+00
    w(2) = 0.0886157460419145274808558830057D+00
    w(3) = 0.40882846955602922608853782625D+00
    w(4) = 0.40882846955602922608853782625D+00
    w(5) = 0.0886157460419145274808558830057D+00
    w(6) = 0.00255578440205624643060629074383D+00

  else if ( n == 7 ) then

    x(1) = -3.75043971772574225630392202571D+00
    x(2) = -2.36675941073454128861885646856D+00
    x(3) = -1.15440539473996812723959775884D+00
    x(4) =  0.0D+00
    x(5) = +1.15440539473996812723959775884D+00
    x(6) = +2.36675941073454128861885646856D+00
    x(7) = +3.75043971772574225630392202571D+00

    w(1) = 0.000548268855972217791621570532802D+00
    w(2) = 0.0307571239675864970396450057164D+00
    w(3) = 0.240123178605012713740161995179D+00
    w(4) = 0.457142857142857142857142857143D+00
    w(5) = 0.240123178605012713740161995179D+00
    w(6) = 0.0307571239675864970396450057164D+00
    w(7) = 0.000548268855972217791621570532802D+00

  else if ( n == 8 ) then

    x(1) = -4.14454718612589433206019783917D+00
    x(2) = -2.80248586128754169911301080618D+00
    x(3) = -1.63651904243510799922544657297D+00
    x(4) = -0.539079811351375108072461918694D+00
    x(5) = +0.539079811351375108072461918694D+00
    x(6) = +1.63651904243510799922544657297D+00
    x(7) = +2.80248586128754169911301080618D+00
    x(8) = +4.14454718612589433206019783917D+00

    w(1) = 0.00011261453837536777039380201687D+00
    w(2) = 0.00963522012078826718691913771988D+00
    w(3) = 0.117239907661759015117137525962D+00
    w(4) = 0.373012257679077349925549534301D+00
    w(5) = 0.373012257679077349925549534301D+00
    w(6) = 0.117239907661759015117137525962D+00
    w(7) = 0.00963522012078826718691913771988D+00
    w(8) = 0.00011261453837536777039380201687D+00

  else if ( n == 9 ) then

    x(1) = -4.51274586339978266756667884317D+00
    x(2) = -3.20542900285646994336567590292D+00
    x(3) = -2.07684797867783010652215614374D+00
    x(4) = -1.02325566378913252482814822581D+00
    x(5) =  0.0D+00
    x(6) = +1.02325566378913252482814822581D+00
    x(7) = +2.07684797867783010652215614374D+00
    x(8) = +3.20542900285646994336567590292D+00
    x(9) = +4.51274586339978266756667884317D+00

    w(1) = 0.0000223458440077465836484639907118D+00
    w(2) = 0.00278914132123176862881344575164D+00
    w(3) = 0.0499164067652178740433414693826D+00
    w(4) = 0.2440975028949394361410220177D+00
    w(5) = 0.406349206349206349206349206349D+00
    w(6) = 0.2440975028949394361410220177D+00
    w(7) = 0.0499164067652178740433414693826D+00
    w(8) = 0.00278914132123176862881344575164D+00
    w(9) = 0.0000223458440077465836484639907118D+00

  else if ( n == 10 ) then

    x(1) =  -4.85946282833231215015516494660D+00
    x(2) =  -3.58182348355192692277623675546D+00
    x(3) =  -2.48432584163895458087625118368D+00
    x(4) =  -1.46598909439115818325066466416D+00
    x(5) =  -0.484935707515497653046233483105D+00
    x(6) =  +0.484935707515497653046233483105D+00
    x(7) =  +1.46598909439115818325066466416D+00
    x(8) =  +2.48432584163895458087625118368D+00
    x(9) =  +3.58182348355192692277623675546D+00
    x(10) = +4.85946282833231215015516494660D+00

    w(1) =  0.0000043106526307182867322209547262D+00
    w(2) =  0.000758070934312217670069636036508D+00
    w(3) =  0.0191115805007702856047383687629D+00
    w(4) =  0.135483702980267735563431657727D+00
    w(5) =  0.344642334932019042875028116518D+00
    w(6) =  0.344642334932019042875028116518D+00
    w(7) =  0.135483702980267735563431657727D+00
    w(8) =  0.0191115805007702856047383687629D+00
    w(9) =  0.000758070934312217670069636036508D+00
    w(10) = 0.0000043106526307182867322209547262D+00

  else

    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'HERMITE_PROBABILIST_SET - Fatal error!'
    write ( *, '(a,i8)' ) '  Illegal value of N = ', n
    write ( *, '(a)' ) '  Legal values are 1 to 10,'
    stop 1
...
!
  ...
  cc = sqrt ( r8_pi ) * gamma ( real ( n, kind = rk ) ) &
    / ( 2.0D+00 ** ( n - 1 ) )

  s = ( 2.0D+00 * real ( n, kind = rk ) + 1.0D+00 ) ** ( 1.0D+00 / 6.0D+00 )
  ...
  do i = 1, ( n + 1 ) / 2
    ...
    if ( i == 1 ) then
      ...
      xval = xval - 1.14D+00 * ( ( real ( n, kind = rk ) ) ** 0.426D+00 ) / xval
...
!  Parameters:
!
!    Output, real ( kind = rk ) P2, the value of H(N)(X).
!
!    Output, real ( kind = rk ) DP2, the value of H'(N)(X).
!
!    Output, real ( kind = rk ) P1, the value of H(N-1)(X).
!
!    Input, real ( kind = rk ) X, the point at which polynomials are evaluated.
!
!    Input, integer N, the order of the polynomial.
...
!
!    Input/output, real ( kind = rk ) D(N).  On input, the diagonal elements of
!    the input matrix.  On output, the eigenvalues in ascending order.  If an
!    error exit is made, the eigenvalues are correct but
!    unordered for indices 1,2,...,IERR-1.
!
!    Input/output, real ( kind = rk ) E(N).  On input, the subdiagonal elements
!    of the input matrix in E(2:N).  E(1) is arbitrary.  On output, E is
!    overwritten.
!
!    Input/output, real ( kind = rk ) Z(N,N).  On input, the transformation
...
!    perform the QL algorithm on a symmetric tridiagonal matrix.
!
...
!    Input, integer EXPON, the exponent.
!
!    Input, real ( kind = rk ) ALPHA, the exponent of (1-X) in the weight factor.
!
!    Input, real ( kind = rk ) BETA, the exponent of (1+X) in the weight factor.
!
!    Output, real ( kind = rk ) VALUE, the value of the integral.
!
  implicit none
  ...
  value = gamma ( 1.0D+00 + c ) * ( &
      s * gamma ( 1.0D+00 + beta  ) * value1 &
    / gamma ( 2.0D+00 + beta  + c ) &
    +     gamma ( 1.0D+00 + alpha ) * value2 &
    / gamma ( 2.0D+00 + alpha + c ) )
  ...
  do i = 1, n
    ...
    if ( i == 1 ) then
      ...
      r3 = 1.0D+00 + 8.0D+00 * beta / &
        ( ( 6.28D+00 + beta ) * real ( n * n, kind = rk ) )
...
!
  do i = 1, n
    b(i) = ( real ( 2 * i - 1, kind = rk ) )
  end do

  do i = 1, n
    c(i) = real ( i - 1, kind = rk ) * ( real ( i - 1, kind = rk ) )
  end do
...
!
    if ( i == 1 ) then
      ...
      xval = 3.0D+00 / ( 1.0D+00 + 2.4D+00 * real ( n, kind = rk ) )

    else if ( i == 2 ) then

      xval = xval + 15.0D+00 / ( 1.0D+00 + 2.5D+00 * real ( n, kind = rk ) )
...
!      function func ( x ).
!
!    Input, real ( kind = rk ) A, the beginning of the integration interval.
!
!    Input, integer N, the order.
!
...
!
!    Output, real ( kind = rk ) RESULT, the approximate value of the integral.
!
  implicit none
...
!
  ...
  do i = 1, n - 1
    beta(i) = 1.0D+00 / real ( 2 * i, kind = rk )
  end do
  beta(1:n-1) = 0.5D+00 &
...
!
  if ( mod ( expon, 2 ) == 0 ) then
    ...
    exact = 2.0D+00 / real ( expon + 1, kind = rk )
...
!
  do i = 1, n
    ...
    c(i) = real ( ( i - 1 ) * ( i - 1 ), kind = rk ) / &
           real ( ( 2 * i - 1 ) * ( 2 * i - 3 ), kind = rk )
  ...
  do i = 1, n
    ...
    if ( i == 1 ) then
      ...
      r = 2.78D+00 / ( 4.0D+00 + real ( n * n, kind = rk ) )

      xtemp = 1.0D+00 - r

    else if ( i == 2 ) then

      r = 1.0D+00 + 0.06D+00 * real ( n - 8, kind = rk ) &
        / real ( n, kind = rk )
      ...
      r = 1.0D+00 + 0.22D+00 * real ( n - 8, kind = rk )  &
        / real ( n, kind = rk )
      ...
      r = 1.0D+00 / ( 1.0D+00 + 0.639D+00 * real ( n - 4, kind = rk ) &
        / ( 1.0D+00 + 0.71D+00 * real ( n - 4, kind = rk ) ) )

      xtemp = xtemp + r * ( xtemp - x(i-2) ) / 0.766D+00

    else if ( i == n ) then

      r = 1.0D+00 / ( 1.0D+00 + 0.22D+00 * real ( n - 8, kind = rk ) &
        / real ( n, kind = rk ) )
...
!
!    A 1-point Lobatto rule may be requested, in which case only the
!    left endpoint is involved.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  do i = 1, n
    x(i) = cos ( r8_pi * real ( i - 1, kind = rk ) / real ( n - 1, kind = rk ) )
  end do
  ...
  do while ( tolerance < maxval ( abs ( x(1:n) - xold(1:n) ) ) )
    ...
    p(1:n,2) = x(1:n)
    do j = 2, n-1
      p(1:n,j+1) = ( real ( 2 * j - 1, kind = rk ) * x(1:n) * p(1:n,j)     &
                   + real (   - j + 1, kind = rk ) *          p(1:n,j-1) ) &
                   / real (     j,     kind = rk )
    end do

    x(1:n) = xold(1:n) - ( x(1:n) * p(1:n,n) - p(1:n,n-1) ) &
             / ( real ( n, kind = rk ) * p(1:n,n) )
  end do

  x(1:n) = x(n:1:-1)
  w(1:n) = 2.0D+00 / ( real ( ( n - 1 ) * n, kind = rk ) * p(1:n,n)**2 )
...
subroutine nc_compute_weights ( n, x_min, x_max, x, w )
...
!    Input, integer N, the order.
!
!    Input, real ( kind = rk ) X_MIN, X_MAX, the endpoints of the interval.
!
!    Input, real ( kind = rk ) X(N), the abscissas.
!
!    Output, real ( kind = rk ) W(N), the weights.
...
!
    yvala = d(n) / real ( n, kind = rk )
    do j = n - 1, 1, -1
      yvala = yvala * x_min + d(j) / real ( j, kind = rk )
    end do
    yvala = yvala * x_min

    yvalb = d(n) / real ( n, kind = rk )
    do j = n - 1, 1, -1
      yvalb = yvalb * x_max + d(j) / real ( j, kind = rk )
    end do
    yvalb = yvalb * x_max
...
!
  ...
  real ( kind = rk ) w(n)
  real ( kind = rk ) x(n)
  real ( kind = rk ) :: x_max = +1.0D+00
  real ( kind = rk ) :: x_min = -1.0D+00
...
!
  ...
  do i = 1, n
    x(i) = ( real ( n - i + 1, kind = rk ) * x_min   &
           + real (     i,     kind = rk ) * x_max ) &
           / real ( n     + 1, kind = rk )
  end do
...
!
  ...
  if ( n == 1 ) then
    ...
    w(9) =    4045.0D+00 / d
!
!  Arithmetic seems to become unreliable at this point!
!
  else if ( n == 10 ) then
...
!   w(1) =    2752477.0D+00 / d
!   w(2) =  - 6603199.0D+00 / d
!   w(3) =   15673880.0D+00 / d
!   w(4) = - 17085616.0D+00 / d
!   w(5) =    8891258.0D+00 / d
!   w(6) =    8891258.0D+00 / d
!   w(7) = - 17085616.0D+00 / d
!   w(8) =   15673880.0D+00 / d
!   w(9) =  - 6603199.0D+00 / d
!   w(10) =   2752477.0D+00 / d

    w(1) =    0.758508873456792D+00
    w(2) =   -1.819664627425049D+00
    w(3) =    4.319301146384676D+00
    w(4) =   -4.708337742504753D+00
    w(5) =    2.450192350088813D+00
    w(6) =    2.450192350087711D+00
    w(7) =   -4.708337742504625D+00
    w(8) =    4.319301146384526D+00
    w(9) =   -1.819664627425028D+00
    w(10) =   0.758508873456790D+00
...
!
  ...
  do i = 1, n
    x(i) = ( real ( 2 * n - 2 * i + 1, kind = rk ) * x_min   &
           + real (         2 * i - 1, kind = rk ) * x_max ) &
           / real ( 2 * n,             kind = rk )
  end do
...
!
!    Output, real ( kind = rk ) R8_FACTORIAL2, the value of the double factorial.
!
  implicit none
...
!
!    Input/output, real ( kind = rk ) A(N), the array to be reversed.
!
  implicit none
...
!
  do i = 1, n
    x(i) = - cos ( 2.0D+00 * r8_pi * real (     i - 1, kind = rk ) &
                                   / real ( 2 * n - 1, kind = rk ) )
  end do
  ...
  do while ( tolerance < maxval ( abs ( x(1:n) - xold(1:n) ) ) )
    ...
    do j = 2, n
      p(2:n,j+1) = ( real ( 2 * j - 1, kind = rk ) * x(2:n) * p(2:n,j)     &
                   + real (   - j + 1, kind = rk ) *          p(2:n,j-1) ) &
                   / real (     j,     kind = rk )
    end do

    x(2:n) = xold(2:n) - ( ( 1.0D+00 - xold(2:n) ) / real ( n, kind = rk ) ) &
      * ( p(2:n,n) + p(2:n,n+1) ) / ( p(2:n,n) - p(2:n,n+1) )

  end do

  w(1) = 2.0D+00 / real ( n * n, kind = rk )
  w(2:n) = ( 1.0D+00 - x(2:n) ) / ( real ( n, kind = rk ) * p(2:n,n) )**2
...
subroutine scqf ( nt, t, mlt, wts, nwts, ndx, swts, st, rule_kind, alpha, beta, a, &
  b )
